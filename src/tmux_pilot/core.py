...
def send_text(
    ...
    *,
    ...
    if wait:
        agent = wait_until_session_ready(
            name,
            timeout=timeout,
            interval=interval,
...
def should_use_profile_mode(
    *,
    ...
    if profile_name or issue is not None or repo or no_agent:
        return True
    if agent:
        return False
    if prompt:
...
def launch_agent_session(
    ...
    *,
    ...
    """Launch an agent command in a tmux session and optionally send a prompt."""
    _launch_agent(session_name, agent, agent_args)
    if prompt:
        send_text(session_name, prompt, wait=True, use_transcript=False)
...
def create_profile_session(
    ...
    *,
    ...
    if not no_agent and profile.agent:
        launch_agent_session(name, profile.agent, prompt=prompt, agent_args=profile.agent_args)
...
def _get_agent_state(
    ...
    *,
    ...
    if not pane_path and session_exists(session_name) and use_transcript:
    ...
    return get_agent_state(
        ...
        working_dir=pane_path if use_transcript else "",
        transcript_path=transcript_path if use_transcript else None,
    )
...
def wait_until_session_ready(
    ...
    *,
    ...
    while True:
        ...
        if pane_path != last_pane_path:
            transcript_path = None
            last_pane_path = pane_path
        ...
        if isinstance(agent_type, str) and agent_type != last_agent_type:
            transcript_path = None
            last_agent_type = agent_type
        if use_transcript and isinstance(agent_type, str) and pane_path and transcript_path is None:
